...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
        ...
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('goods','修改') && user.id == product.yonghuId" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('goods','删除') && user.id == product.yonghuId" class="cuIcon-delete"
	...
	export default {
		...
		methods: {
			...
			async search() {
				...
				if (this.searchForm.goodsUuidNumber) {
                    searchForm['goodsUuidNumber'] = this.searchForm.goodsUuidNumber
                }
