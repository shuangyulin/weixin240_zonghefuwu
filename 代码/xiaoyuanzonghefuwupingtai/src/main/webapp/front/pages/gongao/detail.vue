...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback"
		...
		<view class="container">
			...
			<!-- 名字-->
						<view :style='{"textAlign":"left","fontSize":"45upx","color":"black"}'>{{detail.gongaoName}}
						</view>
   	 ...
   	 <view style="margin-top: 10rpx;"></view>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.gongaoContent"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('gongao', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.gongaoPhoto ? this.detail.gongaoPhoto.split(",") : [];
			},
			// 收藏
			async getgongaoCollection() {
                let params = {
                    page: 1,
                    limit: 1,
                    gongaoId: this.id,
                    yonghuId: this.user.id,
                    gongaoCollectionTypes:'1'
                }
                let res = await this.$api.list(`gongaoCollection`, params);
			...
			async shoucang() {
                ...
                let params = {
                    page: 1,
                    limit: 1,
                    gongaoId: _this.detail.id,
                    yonghuId: _this.user.id,
                    gongaoCollectionTypes:'1'
                }
                let res = await _this.$api.list(`gongaoCollection`, params);
                if (res.data.list.length == 1) {
                    let gongaoCollectionId = res.data.list[0].id;
                    uni.showModal({
                        title: '提示',
                        content: '是否取消',
                        success: async function(res) {
                            if (res.confirm) {
                                await _this.$api.del('gongaoCollection', JSON.stringify([gongaoCollectionId]));
                                _this.$utils.msg('取消成功');
                                _this.getgongaoCollection();
                            }
                        }
                    });
                    return;
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.add('gongaoCollection', {
                                yonghuId: _this.user.id,
                                gongaoId: _this.detail.id,
                                gongaoCollectionTypes:1
                            });
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('gongaoLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					gongaoId: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
                this.$utils.jump(`../gongaoLiuyan/add-or-update?gongaoId=${this.id}`)
            },
		}
	}
</script>
