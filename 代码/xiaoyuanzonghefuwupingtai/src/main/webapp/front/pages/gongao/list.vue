<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.gongaoName" type="text" placeholder="资讯标题"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in gongaoTypesList" :key="index"
    ...
    <view
        ...
        <view class="image-view">
            <image :style='{"borderRadius":"0px","width":"100%","height":"100%"}' mode="aspectFill"
                   class="uni-product-image" :src="product.gongaoPhoto?product.gongaoPhoto.split(',')[0]:''"></image>
        </view>
        <view class="uni-product-price" v-if="product.gongaoNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#999","textAlign":"left"}'>
            <text class="">￥{{product.gongaoNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('gongao','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('gongao','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('gongao','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
		  	...
		  	let gongaoTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'gongao_types'
		  	});
		  	this.gongaoTypesList = gongaoTypes.data.list;
		...
		methods: {
				...
				let params = {
					...
					limit: mescroll.size
							...
							if (this.categoryName != 0) {
                                params.gongaoTypes = this.categoryName
                            }

                let res = await this.$api.list(`gongao`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('gongao', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.gongaoName) {
                    searchForm['gongaoName'] = this.searchForm.gongaoName
                }
				if (this.searchForm.gongaoName) {
					searchForm['gongaoName'] = this.searchForm.gongaoName
				}

                let res = await this.$api.list(`gongao`, searchForm);
				// 如果是第一页数据置空
