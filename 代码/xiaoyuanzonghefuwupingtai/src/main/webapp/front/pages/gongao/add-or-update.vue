        ...
        <form class="app-update-pv">
            <view
                ...
                <input   disabled
                         ...
                         :disabled="ro.gongaoName" type="text" v-model="ruleForm.gongaoName" placeholder="资讯标题"></input>
            </view>
                <view :style='{"boxShadow":"0","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(228, 232, 230, 1)","margin":"0 0 0px 0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"88rpx"}'
                    ...
                    <picker @change="gongaoTypesChange" :value="gongaoTypesIndex" :range="gongaoTypesOptions" range-key="indexName">
                        <view
                                :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"#47EDB5","borderRadius":"0px","color":"#47EDB5","textAlign":"left","borderWidth":"0 0 6rpx","fontSize":"24rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
                                class="uni-input">{{ruleForm.gongaoTypes?ruleForm.gongaoValue:"请选择资讯类型"}}</view>
                    </picker>
                </view>
                <view
                        ...
                        class="cu-form-group" @tap="gongaoPhotoTap">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"#47EDB5","textAlign":"left"}'
                    ...
                    <view class="right-input" style="padding:0;textAlign:left">
                        <image
                                ...
                                class="avator" v-if="ruleForm.gongaoPhoto" :src="ruleForm.gongaoPhoto" mode="aspectFill">
                        </image>
            ...
            <view
                ...
                <textarea :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"#47EDB5","borderRadius":"0px","color":"#47EDB5","textAlign":"left","borderWidth":"0 0 6rpx","fontSize":"24rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx","height":"200rpx"}'
                          :disabled="ro.gongaoContent" v-model="ruleForm.gongaoContent" placeholder="资讯详情"/>
            </view>
...
<script>
    ...
    export default {
		...
		/*下拉框*/
			let gongaoTypesParams = {
                page: 1,
                limit: 100,
                dicCode: 'gongao_types',
            }
			let gongaoTypes = await this.$api.page(`dictionary`, gongaoTypesParams);
			this.gongaoTypesOptions = gongaoTypes.data.list
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`gongao`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.gongaoId){
                this.ruleForm.gongaoId = options.gongaoId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            // 下拉变化
            gongaoTypesChange(e) {
                this.gongaoTypesIndex = e.target.value
                this.ruleForm.gongaoValue = this.gongaoTypesOptions[this.gongaoTypesIndex].indexName
                this.ruleForm.gongaoTypes = this.gongaoTypesOptions[this.gongaoTypesIndex].codeIndex
            },
			...
			gongaoPhotoTap() {
                ...
                this.$api.upload(function(res) {
                    _this.ruleForm.gongaoPhoto = _this.$base.url + 'upload/' + res.file;
                    _this.$forceUpdate();
                    _this.$nextTick(() => {
                        // _this.styleChange()
                    })
            ...
            async onSubmitTap() {
				if ((!this.ruleForm.gongaoName)) {
                    this.$utils.msg(`资讯标题不能为空`);
                    return
                }
				if ((!this.ruleForm.gongaoPhoto)) {
                    this.$utils.msg(`资讯图片不能为空`);
                    return
                }
                if (this.ruleForm.id) {
                    await this.$api.update(`gongao`, this.ruleForm);
                } else {
                    await this.$api.save(`gongao`, this.ruleForm);
                }
